...
class NoticeController extends AppController
{
    ...
    public function isAuthorized($user)
    {
        ...
        if ($this->request->getParam('action') === 'add' || $this->request->getParam('action') === 'index' ||  $this->request->getParam('action') === 'view') {
            return true;
        }

        // The owner of an article can edit and delete it
        if (in_array($this->request->getParam('action'), ['edit', 'delete'])) {
            $noticeId = (int)$this->request->getParam('pass.0');
//
            if ($this->Notice->isOwnedBy($noticeId,$user['Id'])) {
                return true;
            }
        }
